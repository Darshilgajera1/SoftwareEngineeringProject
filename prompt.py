...
TOOL_MAKER_PROMPT = SystemMessage(
    ...
    content=(
        ...
        "4. If the task requires integration with external APIs, ensure you use well-documented and open-source APIs. For example, use APIs like OpenWeatherMap for weather data, or Wikipedia's public API for information. Avoid placeholder or example APIs such as 'https://api.example.com'.\n" 
        " Add an input field in the UI that prompts users to enter their API key (e.g., 'Enter your API key'). Ensure the input field is prominently placed and labeled to clearly indicate its purpose. If you cannot find an appropriate open-source API, clearly document why and suggest alternatives.\n"
